# 02 - Product Vision and Strategy
...
**Pillar 2, Community, Innovation and Careers.** Community, technical articles, blog, events, hackathons, open source, research, student projects, showcases, developer profiles, job listings, employer and recruiter ecosystem.

**Pillar 3, ITES and AI Solutions (secondary).** AI, GenAI, Agentic AI, automation, cloud, DevOps, platform engineering, data engineering, software engineering, AI infrastructure, consulting, digital transformation.
...
**On enterprise sponsorship and employer hiring (added via ADR-0015, `docs/39`):** neither required a fourth pillar. Corporate sponsorship is a distribution channel for Pillar 1's existing catalog, an organisation buying seats, not a new business line. The employer talent pipeline extends Pillar 2's existing employer-and-recruiter ecosystem line above, which was already there and simply hadn't been built out. Positioning discipline holds; only the plan grew.
...
## 9. Access model and how learners pay

The platform offers three ways to access learning, and a learner can use all three at once. Free courses and videos cost nothing and free certifications, once earned through real assessment, are kept forever. A subscription, billed monthly or annually, opens the library of programs, tracks, specializations, live cohorts, and recorded lectures for as long as it is active, and a lapse suspends access without ever deleting progress. A one-time purchase buys a flagship program or premium credential outright, with access for the published duration plus one year. There is no instalment financing; a monthly subscription serves the same need to spread cost, without depending on a financing product whose terms could not be confirmed. The full model is in doc 09.

## 10. What sets the platform apart, stated honestly

Competitors in this market are strong and established, and honesty about that is part of this plan's discipline. What genuinely distinguishes this platform, by design rather than by marketing, is a small set of real differences.

It is a genuine platform rather than a single program, an extensible catalogue where new kinds of learning are configuration rather than a rewrite, so breadth grows without the architecture fighting it. It offers genuinely free credentialed courses with real gated assessment, a real top of funnel that the obvious competitors do not match. It commits to transparent public pricing, resolved on the server and shown plainly, where competitors are often quote-based or vary pricing by cohort, and price transparency is a trust signal their opacity lacks. And it is AI-native in its learning experience, with a memory-carrying tutor, AI-assisted problem solving, and AI mock interviews built on open-source models with learner data kept private, where competitors rely on human-only mentorship.

Where the platform is honestly behind, it is behind on things that time and execution fix rather than architecture: it launches without the years of proven placement outcomes competitors can show, and it does not fabricate them, and it does not yet carry an academic brand partnership, which is planned for a later stage once the platform is mature enough to approach a university as a credible partner. Naming these gaps plainly is more useful to the business than pretending they do not exist.
